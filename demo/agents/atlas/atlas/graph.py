...
async def _memory_call(method: str, params: dict) -> dict | list:
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(f"{MEMORY_ENDPOINT}/{method}", json=params, timeout=30)
...
async def synthesize(state: AtlasState) -> AtlasState:
    ...
    # Build synthesis
    entity_summary = (
        ", ".join(f"{e['name']} ({e['type']})" for e in state["entities"][:10])
