...
async def seed():
    async with httpx.AsyncClient() as c:
        ...
        for name, desc in [
            ...
            e = await call(
                c, "add_entity", {"name": name, "entity_type": "PERSON", "description": desc}
            )
            people[name] = e["id"]
            print(f"  + {name} (PERSON)")
        ...
        for name, desc in [
            ...
            e = await call(
                c, "add_entity", {"name": name, "entity_type": "ORGANIZATION", "description": desc}
            )
            orgs[name] = e["id"]
        ...
        for name, desc in [
            (
                "Lex Fridman Podcast #401",
                "Interview with Sam Altman about AGI and OpenAI's mission",
            ),
            ...
            e = await call(
                c, "add_entity", {"name": name, "entity_type": "EVENT", "description": desc}
            )
            events[name] = e["id"]
            print(f"  + {name} (EVENT)")
        ...
        for role, content in [
            (
                "user",
                "Analyze the Lex Fridman podcast episode #401 with Sam Altman. Extract all people, companies, and key topics discussed.",
            ),
            (
                "assistant",
                "I'll analyze the transcript and extract entities. I found several key people and organizations mentioned in the episode.",
            ),
            ("user", "Great. What are the key relationships between these entities?"),
            (
                "assistant",
                "Sam Altman is the CEO of OpenAI. The episode discusses the $13B Microsoft investment, NVIDIA's role as GPU supplier, and Dario Amodei's departure from OpenAI to found Anthropic. Lex Fridman also references his previous interview with Dario (#380).",
            ),
            ("user", "Can you also capture the main topics discussed?"),
            (
                "assistant",
                "The main topics were: AGI timeline predictions, AI safety concerns, the competitive landscape between OpenAI and Anthropic, NVIDIA's GPU dominance, and the role of open-source models like Meta's LLaMA.",
            ),
        ]:
        ...
        trace = await call(
            ...
            {
                "session_id": sid,
                "task": "Extract entities from Lex Fridman podcast #401 with Sam Altman",
            },
        )
        s1 = await call(
            c,
            ...
            {
                "trace_id": trace["id"],
                "thought": "Parsing transcript for named entities",
                "action": "parse_transcript",
                "observation": "Found 8 people, 7 organizations, 3 events",
            },
        )
        await call(
            c,
            "record_tool_call",
            {
                ...
                "arguments": {"source": "podcast_401_transcript.txt"},
                "status": "success",
                "duration_ms": 2340,
                "result": {"people": 8, "orgs": 7, "events": 3},
            },
        )
        s2 = await call(
            c,
            ...
            {
                "trace_id": trace["id"],
                "thought": "Identifying relationships between entities",
                "action": "relationship_extraction",
                "observation": "Found 25 facts connecting entities",
            },
        )
        await call(
            c,
        ...
        s3 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Storing entities and facts in knowledge graph",
                "action": "store_knowledge",
                "observation": "All entities and facts stored successfully",
            },
        )
        await call(
            c,
            "record_tool_call",
            {
                ...
                "arguments": {"entities": 18, "facts": 25},
                "status": "success",
                "duration_ms": 450,
            },
        )
        await call(
            ...
            {
                "trace_id": trace["id"],
                "outcome": "Extracted 18 entities and 25 facts from podcast #401",
                "success": True,
            },
        )
        ...
        for role, content in [
            ...
            (
                "assistant",
                "I found several recent developments. OpenAI launched GPT-4 Turbo with improved performance and lower costs. They also announced a partnership with Apple for AI integration in iOS.",
            ),
            ("user", "What about Anthropic? Any recent updates?"),
            (
                "assistant",
                "Anthropic released Claude 3.5 Sonnet, which shows strong performance on coding and reasoning benchmarks. They also raised $2B in a Series D round led by Google, bringing total funding to $7.3B.",
            ),
        ...
        trace = await call(
            c,
            "start_trace",
            {"session_id": sid, "task": "Web search: latest AI industry developments"},
        )
        s1 = await call(
            c,
            "add_step",
            {
                "trace_id": trace["id"],
                "thought": "Checking existing entities in shared graph",
                "action": "search_entities",
                "observation": "Found 18 entities from Lenny's research",
            },
        )
        await call(
            c,
            "record_tool_call",
            {
                ...
                "arguments": {"query": "OpenAI Anthropic", "limit": 10},
                "status": "success",
                "duration_ms": 120,
            },
        )
        s2 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Searching web for latest OpenAI news",
                "action": "web_search",
                "observation": "Found 5 relevant articles about GPT-4 Turbo and Apple partnership",
            },
        )
        await call(
            c,
            "record_tool_call",
            {
                ...
                "arguments": {"query": "OpenAI latest news 2025"},
                "status": "success",
                "duration_ms": 890,
            },
        )
        s3 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Enriching entities with new information",
                "action": "enrich_entities",
                "observation": "Updated 3 entities with latest data",
            },
        )
        await call(
            c,
            "record_tool_call",
            {
                "step_id": s3["id"],
                "tool_name": "add_fact",
                "arguments": {"subject": "OpenAI", "predicate": "LAUNCHED", "obj": "GPT-4 Turbo"},
                "status": "success",
                "duration_ms": 65,
            },
        )
        await call(
            ...
            {
                "trace_id": trace["id"],
                "outcome": "Enriched graph with latest AI industry developments from web search",
                "success": True,
            },
        )
        print(f"  + Reasoning trace: {trace['id'][:8]}... (3 steps, 3 tool calls)")
        ...
        for role, content in [
            ...
            (
                "assistant",
                "Enriched Sam Altman with properties: ROLE=CEO, COMPANY=OpenAI, EDUCATION=Stanford dropout",
            ),
            (
                "assistant",
                "Enriched NVIDIA with properties: MARKET_CAP=$3.4T, FOUNDED=1993, HEADQUARTERS=Santa Clara",
            ),
            (
                "assistant",
                "Enriched Anthropic with properties: FUNDING=$7.3B, FOUNDED=2021, HEADQUARTERS=San Francisco",
            ),
        ...
        trace = await call(
            c, "start_trace", {"session_id": sid, "task": "Enrich entity: Sam Altman"}
        )
        s1 = await call(
            c,
            "add_step",
            {
                "trace_id": trace["id"],
                "thought": "Looking up Sam Altman in knowledge graph",
        ...
        s2 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Adding structured properties from data pipeline",
        ...
        await call(
            ...
            "record_tool_call",
            {
                ...
                "arguments": {
                    "subject": "Sam Altman",
                    "predicate": "EDUCATION",
                    "obj": "Stanford University (dropped out)",
                },
                "status": "success",
                "duration_ms": 42,
            },
        ...
        await call(
            ...
            {
                "trace_id": trace["id"],
                "outcome": "Added 3 structured properties to Sam Altman",
                "success": True,
            },
        )
        ...
        s1 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Running data pipeline for NVIDIA",
                "action": "enrich_entity",
            },
        )
        await call(
        ...
        await call(
            ...
            {
                "trace_id": trace["id"],
                "outcome": "Added 3 structured properties to NVIDIA",
                "success": True,
            },
        )
        ...
        for role, content in [
            (
                "user",
                "Synthesize everything we know about the AI industry landscape from all agents' research.",
            ),
            (
                "assistant",
        ...
        trace = await call(
            c,
            "start_trace",
            {"session_id": sid, "task": "Synthesize AI industry landscape from all agents"},
        )
        s1 = await call(
            c,
            "add_step",
            {
                "trace_id": trace["id"],
                "thought": "Gathering entities from shared knowledge graph",
                "action": "gather_entities",
                "observation": "Found 18 entities across Person, Organization, and Event types",
            },
        )
        await call(
            c,
        ...
        s2 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Collecting reasoning traces from all agents",
                "action": "gather_traces",
                "observation": "Found 4 traces: lenny(1), scout(1), forge(2)",
            },
        )
        await call(
            c,
        ...
        s3 = await call(
            ...
            {
                "trace_id": trace["id"],
                "thought": "Synthesizing cross-agent knowledge into unified report",
                "action": "synthesize",
                "observation": "Generated comprehensive AI industry landscape report",
            },
        )
        await call(
            c,
            ...
            {
                "step_id": s3["id"],
                "tool_name": "generate_synthesis",
                "arguments": {"entity_count": 18, "trace_count": 4, "fact_count": 25},
                "status": "success",
                "duration_ms": 320,
            },
        )
        await call(
            ...
            {
                "trace_id": trace["id"],
                "outcome": "Synthesized AI industry landscape from 4 agents, 18 entities, 25+ facts",
                "success": True,
            },
        )
        print(f"  + Reasoning trace: {trace['id'][:8]}... (3 steps, 3 tool calls)")
