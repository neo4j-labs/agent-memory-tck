...
Make sure `tsconfig.json` has `"moduleResolution": "bundler"` or
`"NodeNext"`. The package uses subpath exports, which require modern
module resolution.
...
* Ask on the
  ...
  support channel).
* File a https://github.com/neo4j-labs/agent-memory-tck/issues[GitHub
  issue]. Include the `requestId` from any failing request.
* For suspected security issues, see
  https://github.com/neo4j-labs/agent-memory-tck/blob/main/SECURITY.md[SECURITY.md].
