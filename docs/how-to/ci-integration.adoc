...
== GitHub Actions: HTTP Bridge (TypeScript/Go/C#)
...
TIP: Pre-building the C# project with `dotnet build` before `dotnet run --no-build` ensures fast startup. The health check loop replaces a fixed `sleep` and is more reliable in CI environments.
