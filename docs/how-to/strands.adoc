...
* **`SnapshotStorage`** — session state persists to a NAMS conversation
  ...
  plus `appData`, plus manifests) rides on synthetic `role: "system"`
  marker messages whose per-message metadata carries the JSON blob.
...
| Agent state (`Snapshot.data`, non-message fields) | synthetic `role: "system"` message with content `__strands_state__:{snapshotId}`, JSON in `message.metadata.strands_state`
| Application state (`Snapshot.appData`) | inside the same synthetic message's metadata blob
| Tool calls | `ToolCall` nodes attached to a `ReasoningStep`
| Per-turn reasoning | `ReasoningStep` nodes attached to the Conversation
| Manifests (snapshot metadata) | synthetic `role: "system"` message with content `__strands_manifest__:{scopeId}`
|===

NAMS exposes no endpoint to update a Conversation's `metadata` after
creation, so per-snapshot state lives in synthetic messages whose
per-message metadata carries the JSON blob. Each call to `saveSnapshot`
writes one synthetic message. `listSnapshotIds` walks the message list,
filters by the `__strands_state__:` content prefix, and dedupes by
`snapshotId` (the last-write-wins for the `isLatest` flag).

Synthetic markers are filtered out before being handed back to Strands
on `loadSnapshot`, so the framework never replays them. Consumers
walking the raw message list themselves (chat UIs, Cypher queries,
custom analytics) should filter on the prefixes too — the integration
exports `isSyntheticStrandsMessage` and `SYNTHETIC_MESSAGE_PREFIXES` for
convenience:
...
The JSON blob value is a single string — the REST transport's
snake_case ↔ camelCase casing layer only transforms keys, never string
values, so the blob round-trips losslessly.
