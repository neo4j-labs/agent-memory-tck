...
The neo4j-agent-memory ecosystem is expanding beyond a single Python package. TypeScript, Go, C#, and hosted service implementations need a shared definition of "compatible." Without a TCK:
...
* The same test suite validates Python, TypeScript, Go, C#, or any other implementation.
...
Native test suites in TypeScript (Vitest), Go (`testing`), and C# (xUnit) are planned as *secondary validation*. The Python suite remains the single source of truth because:
...
The TCK, TypeScript client, Go client, C# client, and demo all live in one repository. This enables:
...
* *Cross-references*: TypeScript, Go, and C# test data mirror the Python fixtures exactly.
