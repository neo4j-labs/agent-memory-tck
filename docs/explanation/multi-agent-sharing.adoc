...
The repository includes a four-agent demo proving this architecture:
...
Each agent runs as a separate service (Cloud Run or Docker container), all connecting to the same Neo4j instance. The TCK guarantees that Lenny's Python writes are readable by Scout's TypeScript reads and Forge's Go reads — because they all implement the same schema.
